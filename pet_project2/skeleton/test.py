...
def test_issued_credentials(input_issuer, input_anoncred, input_issue_request):
    ...
    server_sk = jsonpickle.decode(input_issuer.get_serialized_secret_key().decode("utf-8"))
...
def test_correct_registration(input_client, input_server):
    ...
    cred = jsonpickle.decode(sigma_serial.decode('utf-8')).get("sigma")
...
def test_INcorrect_registration(input_client, input_server):
    ...
    request = jsonpickle.decode(request.decode("utf-8"))
    r = request.get("r")
