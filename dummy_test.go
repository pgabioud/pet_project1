...
func test(ciruitID CircuitID, t *testing.T) {
	...
	for _, p := range dummyProtocol {
		fmt.Println(p, "completed with output", p.Output)
		if p.Output != TestCircuits[ciruitID-1].ExpOutput {
			t.Errorf("Party %d got wrong answer: %d when expected output was: %d", p.ID, p.Output, TestCircuits[ciruitID-1].ExpOutput)
		}
	}
