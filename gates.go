...
func evaluate(cep *DummyProtocol, secrets map[PartyID]uint64, s int64) {
	...
	for _, op := range TestCircuits[cep.circuitID-1].Circuit {
		...
		case *Mult:
			//1. compute (x-a)
			cep.Output = uint64(mod(int64(wire[op.(*Mult).In1])-int64(cep.Beavers[0]), s))
			Revealgate(cep, s)
			xa := cep.Output
			//2. compute (y-b)
			cep.Output = uint64(mod(int64(wire[op.(*Mult).In2])-int64(cep.Beavers[1]), s))
			Revealgate(cep, s)
			yb := cep.Output
			//3. comp z = c + x*(y-b) + y*(x-a) - (x-a)*(y-b)
			z := uint64(mod(int64(cep.Beavers[2])+int64(wire[op.(*Mult).In1]*yb)+int64(wire[op.(*Mult).In2]*xa), s))
			if cep.ID == 0 {
				z = uint64(mod(int64(z)-int64(xa*yb), s))
			}
			wire[op.Output()] = z
